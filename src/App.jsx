...
const shell = {
  page: "min-h-screen overflow-x-hidden bg-[#070707] pb-24 text-white md:pb-0",
  panel: "overflow-hidden rounded-[2rem] border border-[#c8a24a]/30 bg-[linear-gradient(145deg,rgba(200,162,74,0.10),rgba(15,15,15,0.96)_32%,rgba(0,0,0,0.98)_100%)] text-white shadow-2xl shadow-black/60 ring-1 ring-white/10",
  card: "rounded-3xl border border-[#c8a24a]/25 bg-[linear-gradient(145deg,rgba(255,255,255,0.14),rgba(255,255,255,0.045))] p-6 shadow-xl shadow-black/35 backdrop-blur transition hover:-translate-y-1 hover:border-[#c8a24a]/55 hover:bg-white/[0.16]",
};

function SectionHeader({ eyebrow, title, text }) {
  return (
    <div className="mx-auto mb-10 max-w-3xl text-center">
      {eyebrow && <p className="mb-3 text-sm font-semibold uppercase tracking-[0.3em] text-[#c8a24a]">{eyebrow}</p>}
      <h2 className="text-3xl font-bold tracking-tight text-white md:text-5xl">
        <span className="bg-gradient-to-r from-white via-[#f7f1df] to-[#c8a24a] bg-clip-text text-transparent">{title}</span>
      </h2>
      {text && <p className="mt-5 text-base leading-7 text-white md:text-lg md:leading-8">{text}</p>}
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`group flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left text-sm font-semibold shadow-sm transition ${
        active
          ? "bg-gradient-to-r from-white to-[#ead8a6] text-slate-950 shadow-xl shadow-black/20"
          : "border border-white/20 bg-white/[0.12] text-white hover:-translate-y-0.5 hover:border-white/30 hover:bg-white/[0.18] hover:shadow-lg"
      }`}
    >
...
function CTAButton({ children, onClick, variant = "primary" }) {
  const classes = variant === "primary"
    ? "bg-gradient-to-r from-white to-[#ead8a6] text-slate-950 hover:from-[#f3e7c5] hover:to-white shadow-xl shadow-black/20"
    : "border border-white/20 bg-white/[0.12] text-white hover:bg-white/[0.12]";
  return (
...
export default function ElevatedAutoDieselRepair() {
  ...
  return (
    <div className={shell.page}>
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_top_left,rgba(200,162,74,0.12),transparent_32%),radial-gradient(circle_at_bottom_right,rgba(255,255,255,0.06),transparent_28%),linear-gradient(135deg,rgba(255,255,255,0.03),transparent_40%)]" />
      <header className="sticky top-0 z-50 border-b border-white/20 bg-[#070707]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 md:px-8 md:py-4">
          ...
          <nav className="hidden items-center gap-2 text-sm font-medium text-white md:flex">
            {pages.slice(0, 6).map((page) => (
              <button key={page.id} onClick={() => setActivePage(page.id)} className={`rounded-2xl px-4 py-2.5 transition ${activePage === page.id ? "bg-gradient-to-r from-white to-[#ead8a6] text-slate-950 shadow-lg" : "hover:bg-white/10 hover:text-white"}`}>{page.label}</button>
            ))}
      ...
      <main className="relative mx-auto grid max-w-7xl gap-6 px-4 py-6 md:grid-cols-[280px_1fr] md:px-8 md:py-8">
        <aside className="md:sticky md:top-24 md:self-start">
          <div className="rounded-[2rem] border border-white/20 bg-white/[0.12] p-4 shadow-2xl shadow-black/30 backdrop-blur-xl">
            <p className="mb-3 px-2 text-xs font-semibold uppercase tracking-[0.25em] text-[#c8a24a]">Navigate</p>
      ...
      <footer className="relative border-t border-white/20 bg-[#070707] px-5 py-10 md:px-8">
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-white/20 bg-[#070707]/95 p-3 shadow-2xl backdrop-blur-xl md:hidden">
        <div className="mx-auto grid max-w-md grid-cols-2 gap-3">
          <a href="tel:+19709660473" className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-white to-[#ead8a6] px-4 py-3 text-sm font-semibold text-slate-950"><Phone className="mr-2 h-4 w-4" /> Call Now</a>
...
function PageShell({ children, dark = false }) {
  return <section className={dark ? "overflow-hidden rounded-[2rem] border border-white/20 bg-slate-950 text-white shadow-2xl shadow-black/40" : shell.panel}>{children}</section>;
}
        ...
        <div className="relative grid items-center gap-8 p-5 sm:p-8 lg:grid-cols-2 lg:p-12">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <p className="rounded-full border border-white/15 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-white sm:px-4 sm:text-sm">Premium Mobile Auto & Diesel Repair</p>
              <p className="rounded-full border border-white/15 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-white sm:px-4 sm:text-sm">Larimer & Weld County</p>
            ...
            <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight text-white sm:text-5xl lg:text-6xl">
              <span className="bg-gradient-to-r from-white via-[#f7f1df] to-[#c8a24a] bg-clip-text text-transparent">Premium Auto, Diesel & Mobile Repair</span>
              <span className="block text-white">Built Around Your Schedule.</span>
            </h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-white sm:text-lg sm:leading-8">Professional mobile auto repair, diesel service, RV repair, roadside assistance, hydraulic cylinder repair, welding fabrication, and light autobody repair throughout Loveland, Larimer County, Weld County, and Northern Colorado.</p>
            <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:flex xl:flex-wrap">
              <a href="tel:+19709660473" className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-white to-[#ead8a6] px-4 py-3 text-sm font-semibold text-slate-950 shadow-sm transition hover:bg-slate-100"><Phone className="mr-2 h-4 w-4" /> Call 970-966-0473</a>
          ...
          <div className="rounded-[2rem] border border-[#c8a24a]/25 bg-gradient-to-br from-white/[0.14] to-white/[0.04] p-3 shadow-2xl shadow-black/50 sm:p-4">
            <div className="relative overflow-hidden rounded-[1.5rem] border border-[#c8a24a]/25 bg-gradient-to-br from-slate-700 via-slate-900 to-black shadow-2xl">
              ...
              <div className="absolute bottom-4 left-4 right-4">
                <p className="mb-2 w-fit rounded-full border border-[#c8a24a]/40 bg-[#c8a24a]/20 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#f3e7c5] backdrop-blur">Mobile Service Available</p>
                <h3 className="text-xl font-bold text-white sm:text-2xl">Heavy-Duty Help, Wherever You Are</h3>
                <p className="mt-1 text-sm text-white">Serving Larimer & Weld County communities.</p>
              </div>
            </div>
...
function TrustCard({ icon: Icon, text }) {
  return <div className="rounded-2xl border border-white/20 bg-[#070707]/25 p-4 shadow-inner shadow-white/5"><Icon className="mb-2 h-5 w-5 text-[#c8a24a]" /><p className="text-sm text-white">{text}</p></div>;
}
    ...
    <PageShell>
      <div className="p-6 text-center md:p-12">
        ...
        {fiveStarReviews.length > 0 && (
          <div className="mx-auto mt-8 max-w-3xl rounded-3xl border border-white/20 bg-gradient-to-br from-white/[0.18] to-white/[0.06] p-6 shadow-sm md:p-8">
            <div className="mb-5 flex justify-center gap-1 text-[#c8a24a]">{[1, 2, 3, 4, 5].map((star) => <Star key={star} className="h-6 w-6 fill-current" />)}</div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">{reviewPlatforms.map((platform) => <a key={platform.name} href={platform.url} target="_blank" rel="noopener noreferrer" className="group rounded-3xl border border-white/20 bg-gradient-to-br from-white/[0.18] to-white/[0.06] p-6 text-left shadow-sm transition hover:-translate-y-1 hover:bg-white/[0.18]"><div className="flex items-start justify-between gap-4"><div><p className="text-xl font-bold text-white">{platform.name}</p><p className="mt-2 leading-7 text-white">{platform.description}</p></div><ArrowRight className="h-5 w-5 shrink-0 text-slate-400 transition group-hover:translate-x-1 group-hover:text-white" /></div><p className="mt-5 text-sm font-semibold uppercase tracking-[0.2em] text-[#c8a24a]">{platform.cta}</p></a>)}</div>
      </div>
    </PageShell>
  );
}
...
function StatCard({ icon: Icon, value, label }) {
  return <div className="rounded-3xl border border-white/20 bg-gradient-to-br from-white/[0.18] to-white/[0.06] p-6"><Icon className="mx-auto mb-3 h-7 w-7 text-[#c8a24a]" /><p className="text-3xl font-bold text-white">{value}</p><p className="mt-2 text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">{label}</p></div>;
}
  ...
  return (
    <PageShell>
      <div className="p-6 md:p-12">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{services.map((service) => <motion.div key={service.title} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className={shell.card}><div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-slate-950"><Wrench className="h-5 w-5" /></div><h3 className="text-xl font-bold text-white">{service.title}</h3><p className="mt-3 leading-7 text-white">{service.description}</p></motion.div>)}</div>
  ...
  return (
    <PageShell>
      <div className="p-6 md:p-12">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{galleryImages.map((image) => <motion.article key={image.src} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-3xl border border-white/20 bg-white/[0.12] shadow-sm transition hover:-translate-y-1 hover:bg-white/[0.18] hover:shadow-xl"><img src={image.src} alt={`${image.title} in Larimer County, CO`} className="h-72 w-full object-cover" /><div className="p-6"><p className="mb-2 text-xs font-semibold uppercase tracking-[0.25em] text-[#c8a24a]">{image.category}</p><h3 className="text-xl font-bold text-white">{image.title}</h3><p className="mt-3 leading-7 text-white">{image.caption}</p></div></motion.article>)}</div>
      </div>
    </PageShell>
  );
}
  ...
  return (
    <PageShell>
      <div className="p-6 md:p-12">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{serviceAreas.map((area) => <div key={area} className="flex items-center gap-3 rounded-2xl border border-white/20 bg-white/[0.12] p-5 shadow-sm"><MapPin className="h-5 w-5 text-[#c8a24a]" /><span className="font-semibold text-white">{area}</span></div>)}</div>
      </div>
    </PageShell>
  );
}
  ...
  return (
    <PageShell>
      <div className="p-6 md:p-12">
        ...
        <div className="space-y-4">{faqs.map((item) => <div key={item.question} className="rounded-3xl border border-white/20 bg-white/[0.12] p-7 shadow-sm"><h3 className="text-xl font-bold text-white">{item.question}</h3><p className="mt-3 leading-7 text-white">{item.answer}</p></div>)}</div>
      </div>
    </PageShell>
  );
}
  ...
  return (
    <PageShell dark>
      <div className="grid gap-8 p-6 md:grid-cols-2 md:p-12">
        <div><p className="mb-3 text-sm font-semibold uppercase tracking-[0.3em] text-[#c8a24a]">Request Service</p><h2 className="text-4xl font-bold tracking-tight md:text-5xl">Need Reliable, Professional Repair Without the Hassle?</h2><p className="mt-6 text-lg leading-8 text-white">Contact Elevated Auto & Diesel Repair for mobile auto repair, diesel repair, RV repair, transmission service, hydraulic cylinder repair, welding fabrication, light autobody repair, or roadside assistance in Larimer and Weld County.</p><div className="mt-8 space-y-4 text-white"><p className="flex items-center gap-3"><Phone className="h-5 w-5 text-[#c8a24a]" /> 970-966-0473</p><p className="flex items-center gap-3"><Mail className="h-5 w-5 text-[#c8a24a]" /> elevateddieselrepair@gmail.com</p><p className="flex items-center gap-3"><MapPin className="h-5 w-5 text-[#c8a24a]" /> 1815 Co Rd 13C, Loveland, CO 80537</p></div><div className="mt-6 overflow-hidden rounded-3xl border border-white/20"><iframe title="Google Map Location" src="https://www.google.com/maps?q=1815+Co+Rd+13C+Loveland+CO+80537&output=embed" width="100%" height="250" style={{ border: 0 }} allowFullScreen="" loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe></div></div>
