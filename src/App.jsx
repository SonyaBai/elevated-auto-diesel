...
import { Phone, MapPin, Wrench, Truck, ShieldCheck, Star, Clock, CheckCircle, Mail, ArrowRight, CreditCard, Image, HelpCircle, Home } from "lucide-react";
...
function SectionHeader({ eyebrow, title, text }) {
  return (
    <div className="mx-auto mb-10 max-w-3xl text-center">
      ...
      {text && <p className="mt-5 text-base leading-7 text-slate-600 md:text-lg md:leading-8">{text}</p>}
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`group flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left text-sm font-semibold shadow-sm transition ${
        active ? "bg-slate-950 text-white shadow-xl" : "border border-slate-200 bg-white text-slate-800 hover:-translate-y-0.5 hover:border-slate-950 hover:shadow-md"
      }`}
    >
      <span className="flex items-center gap-3"><Icon className="h-4 w-4" /> {page.label}</span>
      <ArrowRight className={`h-4 w-4 transition ${active ? "text-white" : "text-slate-400 group-hover:translate-x-1 group-hover:text-slate-950"}`} />
    </button>
  );
}
...
function CTAButton({ children, onClick, variant = "primary" }) {
  ...
  return (
    <button onClick={onClick} className={`inline-flex min-w-0 items-center justify-center rounded-2xl px-4 py-3 text-center text-sm font-semibold shadow-sm transition sm:px-6 ${classes}`}>
...
export default function ElevatedAutoDieselRepair() {
  ...
  const renderPage = () => {
    switch (activePage) {
      case "reviews":
        return <ReviewsPage setActivePage={setActivePage} activeReview={activeReview} setActiveReview={setActiveReview} />;
      case "services":
      ...
      case "faq":
        return <FAQPage />;
      case "contact":
        return <ContactPage submitted={submitted} setSubmitted={setSubmitted} />;
      default:
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-slate-50 pb-24 text-slate-950 md:pb-0">
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 md:px-8 md:py-4">
          <button onClick={() => setActivePage("home")} className="flex min-w-0 items-center gap-3">
            <img src="/images/elevated-auto-diesel-logo-phone.png" alt="Elevated Auto & Diesel Repair logo" className="h-10 w-auto max-w-[190px] rounded-xl bg-white object-contain sm:h-12 sm:max-w-[260px] md:h-14 md:max-w-none" />
          </button>
          <div className="flex shrink-0 items-center gap-2 md:hidden">
            ...
            <button onClick={() => setActivePage("contact")} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-950 shadow-sm">Book</button>
          </div>
          <nav className="hidden items-center gap-2 text-sm font-medium text-slate-700 md:flex">
            {pages.slice(0, 6).map((page) => <button key={page.id} onClick={() => setActivePage(page.id)} className={`rounded-2xl px-4 py-2.5 transition ${activePage === page.id ? "bg-slate-950 text-white" : "hover:bg-slate-100 hover:text-slate-950"}`}>{page.label}</button>)}
      ...
      <main className="mx-auto grid max-w-7xl gap-6 px-4 py-6 md:grid-cols-[280px_1fr] md:px-8 md:py-8">
        <aside className="md:sticky md:top-24 md:self-start">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm">
            <p className="mb-3 px-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">Navigate</p>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 p-3 shadow-2xl backdrop-blur md:hidden">
        <div className="mx-auto grid max-w-md grid-cols-2 gap-3">
          <a href="tel:+19709660473" className="inline-flex items-center justify-center rounded-2xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white"><Phone className="mr-2 h-4 w-4" /> Call Now</a>
          <button onClick={() => setActivePage("contact")} className="inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-950">Book Service</button>
        </div>
      </div>
    </div>
  );
...
function PageShell({ children, dark = false }) {
  return <section className={`overflow-hidden rounded-[2rem] shadow-sm ${dark ? "bg-slate-950 text-white" : "border border-slate-200 bg-white text-slate-950"}`}>{children}</section>;
}
...
function TrustCard({ icon: Icon, text }) {
  return <div className="rounded-2xl border border-white/10 bg-white/5 p-4"><Icon className="mb-2 h-5 w-5" /><p className="text-sm text-slate-300">{text}</p></div>;
}
      ...
      <div className="p-6 text-center md:p-12">
        ...
        {fiveStarReviews.length > 0 && (
          <div className="mx-auto mt-8 max-w-3xl rounded-3xl border border-slate-200 bg-slate-50 p-6 shadow-sm md:p-8">
            <div className="mb-5 flex justify-center gap-1 text-slate-950">{[1, 2, 3, 4, 5].map((star) => <Star key={star} className="h-6 w-6 fill-current" />)}</div>
            ...
            <div className="mt-6 flex justify-center gap-2">{fiveStarReviews.map((review, index) => <button key={`${review.name}-${index}`} type="button" onClick={() => setActiveReview(index)} aria-label={`View review ${index + 1}`} className={`h-2.5 rounded-full transition-all ${index === activeReview ? "w-8 bg-slate-950" : "w-2.5 bg-slate-300"}`} />)}</div>
          </div>
        )}
        <div className="mt-8 grid gap-4 md:grid-cols-2">{reviewPlatforms.map((platform) => <a key={platform.name} href={platform.url} target="_blank" rel="noopener noreferrer" className="group rounded-3xl border border-slate-200 bg-white p-6 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xl font-bold text-slate-950">{platform.name}</p><p className="mt-2 leading-7 text-slate-600">{platform.description}</p></div><ArrowRight className="h-5 w-5 shrink-0 text-slate-400 transition group-hover:translate-x-1 group-hover:text-slate-950" /></div><p className="mt-5 text-sm font-semibold uppercase tracking-[0.2em] text-slate-950">{platform.cta}</p></a>)}</div>
      </div>
    </PageShell>
  );
}
...
function StatCard({ icon: Icon, value, label }) {
  return <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6"><Icon className="mx-auto mb-3 h-7 w-7 text-slate-950" /><p className="text-3xl font-bold text-slate-950">{value}</p><p className="mt-2 text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">{label}</p></div>;
}
  ...
  return (
    <PageShell>
      <div className="p-6 md:p-12">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{services.map((service) => <motion.div key={service.title} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-950 text-white"><Wrench className="h-5 w-5" /></div><h3 className="text-xl font-bold text-slate-950">{service.title}</h3><p className="mt-3 leading-7 text-slate-600">{service.description}</p></motion.div>)}</div>
  ...
  return (
    <PageShell>
      <div className="p-6 md:p-12">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{galleryImages.map((image) => <motion.article key={image.src} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><img src={image.src} alt={`${image.title} in Larimer County, CO`} className="h-72 w-full object-cover" /><div className="p-6"><p className="mb-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">{image.category}</p><h3 className="text-xl font-bold text-slate-950">{image.title}</h3><p className="mt-3 leading-7 text-slate-600">{image.caption}</p></div></motion.article>)}</div>
      </div>
    </PageShell>
  );
}
  ...
  return (
    <PageShell>
      <div className="grid items-center gap-12 p-6 md:grid-cols-2 md:p-12">
        <div><p className="mb-3 text-sm font-semibold uppercase tracking-[0.3em] text-slate-500">About</p><h2 className="text-4xl font-bold tracking-tight text-slate-950 md:text-5xl">A Higher Standard for Auto Repair in Larimer County.</h2><p className="mt-6 text-lg leading-8 text-slate-600">Elevated Auto & Diesel Repair was built for drivers who want more than a basic repair experience. We combine skilled diagnostics, clean workmanship, and mobile convenience to make vehicle repair simpler, faster, and more professional.</p><p className="mt-5 text-lg leading-8 text-slate-600">Whether you need engine repair, diesel support, RV service, transmission work, hydraulic cylinder repair, welding fabrication, or light autobody repair, our team brings dependable service directly to you.</p><div className="mt-8"><CTAButton onClick={() => setActivePage("contact")}>Schedule Service</CTAButton></div></div>
        <div className="grid gap-5">{[[Clock, "Convenient Mobile Service", "We save customers time by bringing repair support to homes, workplaces, job sites, and roadside locations."], [ShieldCheck, "Trust-First Workmanship", "Transparent communication, reliable diagnostics, and high-quality repairs from start to finish."], [Star, "Proven Local Reputation", "Backed by 70+ excellent Google reviews and trusted by drivers across Larimer and Weld County."]].map(([Icon, title, text]) => <div key={title} className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm"><Icon className="mb-4 h-7 w-7 text-slate-950" /><h3 className="text-xl font-bold">{title}</h3><p className="mt-3 leading-7 text-slate-600">{text}</p></div>)}</div>
      </div>
    </PageShell>
  );
}
  ...
  return (
    <PageShell>
      <div className="p-6 md:p-12">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{serviceAreas.map((area) => <div key={area} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><MapPin className="h-5 w-5 text-slate-950" /><span className="font-semibold">{area}</span></div>)}</div>
      </div>
    </PageShell>
  );
}
